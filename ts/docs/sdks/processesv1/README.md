...
async function run() {
  ...
  const result = await sdk.processesV1.getProcessInfoDeprecated(processId, appId);
...
**Promise<[models.GetProcessInfoDeprecatedResponse](../../models/getprocessinfodeprecatedresponse.md)>**
### Errors
...
async function run() {
  ...
  const result = await sdk.processesV1.getStoppedProcesses(appId, region);

...
export class ManagementV1 extends ClientSDK {
    ...
    async sendVerificationEmail(
        ...
        const payload$ = schemas$.parse(
            input,
            (value$) => models.VerificationEmailRequest$.outboundSchema.parse(value$),
        ...
        if (this.matchResponse(response, 200, "application/json")) {
            ...
            const result = schemas$.parse(
                responseBody,
                (val$) => {
                    return models.SendVerificationEmailResponse$.inboundSchema.parse({
                        ...responseFields$,
                        VerificationEmailResponse: val$,
                    });
                },
                "Response validation failed"
            );
            return result;
        } else if (this.matchResponse(response, [401, 500], "application/json")) {
            const responseBody = await response.json();
            const result = schemas$.parse(
                responseBody,
                (val$) => {

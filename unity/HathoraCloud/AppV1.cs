...
namespace HathoraCloud
{
    ...
    public class AppV1: IAppV1
    {
        ...
        public async Task<CreateAppResponse> CreateAppAsync(AppConfig request)
        {
            ...
            if((response.StatusCode == 401) || (response.StatusCode == 422) || (response.StatusCode == 500))
            {
                if(Utilities.IsContentTypeMatch("application/json",response.ContentType))
                {
        ...
        public async Task<GetAppsResponse> GetAppsAsync()
        {
            ...
            if((response.StatusCode == 200))
            {
                if(Utilities.IsContentTypeMatch("application/json",response.ContentType))
                {
                    response.Classes = JsonConvert.DeserializeObject<List<ApplicationWithLatestDeploymentAndBuild>>(httpResponse.downloadHandler.text, new JsonSerializerSettings(){ NullValueHandling = NullValueHandling.Ignore, Converters = new JsonConverter[] { new FlexibleObjectDeserializer(), new DateOnlyConverter(), new EnumSerializer() }});
                }
